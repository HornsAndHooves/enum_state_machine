...
class PathCollectionByDefaultTest < MiniTest::Test
  def setup
    ...
    @paths = EnumStateMachine::PathCollection.new(@object, @machine)
  end
...
class PathCollectionTest < MiniTest::Test
  ...
  def test_should_raise_exception_if_invalid_option_specified
    exception = assert_raises(ArgumentError) {EnumStateMachine::PathCollection.new(@object, @machine, :invalid => true)}
    assert_equal 'Invalid key(s): invalid', exception.message
  end
  
  def test_should_raise_exception_if_invalid_from_state_specified
    exception = assert_raises(IndexError) {EnumStateMachine::PathCollection.new(@object, @machine, :from => :invalid)}
    assert_equal ':invalid is an invalid name', exception.message
  end
  
  def test_should_raise_exception_if_invalid_to_state_specified
    exception = assert_raises(IndexError) {EnumStateMachine::PathCollection.new(@object, @machine, :to => :invalid)}
    assert_equal ':invalid is an invalid name', exception.message
  end
end
...
class PathCollectionWithPathsTest < MiniTest::Test
  ...
  def test_should_enumerate_paths
    assert_equal [[
      ...
      EnumStateMachine::Transition.new(@object, @machine, :shift_up, :idling, :first_gear)
    ]], @paths
  end
...
class PathWithGuardedPathsTest < MiniTest::Test
  ...
  def test_should_not_enumerate_paths_if_guard_enabled
    assert_equal [], EnumStateMachine::PathCollection.new(@object, @machine)
  end
  
  def test_should_enumerate_paths_if_guard_disabled
    paths = EnumStateMachine::PathCollection.new(@object, @machine, :guard => false)
    assert_equal [[
      EnumStateMachine::Transition.new(@object, @machine, :ignite, :parked, :idling)
    ]], paths
  end
...
class PathCollectionWithFromStateTest < MiniTest::Test
  def setup
    ...
    @paths = EnumStateMachine::PathCollection.new(@object, @machine, :from => :idling)
  end
  
  def test_should_generate_paths_from_custom_from_state
    assert_equal [[
      EnumStateMachine::Transition.new(@object, @machine, :park, :idling, :parked)
    ]], @paths
  end
...
class PathCollectionWithToStateTest < MiniTest::Test
  def setup
    ...
    @paths = EnumStateMachine::PathCollection.new(@object, @machine, :to => :idling)
  end
  
  def test_should_stop_paths_once_target_state_reached
    assert_equal [
      [EnumStateMachine::Transition.new(@object, @machine, :ignite, :parked, :idling)],
      [EnumStateMachine::Transition.new(@object, @machine, :shift_up, :parked, :idling)]
    ], @paths
  end
end
...
class PathCollectionWithDeepPathsTest < MiniTest::Test
  def setup
    ...
    @paths = EnumStateMachine::PathCollection.new(@object, @machine, :to => :idling, :deep => true)
  end
  ...
  def test_should_allow_target_to_be_reached_more_than_once_per_path
    assert_equal [
      ...
      [
        EnumStateMachine::Transition.new(@object, @machine, :ignite, :parked, :idling),
        EnumStateMachine::Transition.new(@object, @machine, :shift_up, :idling, :first_gear),
        EnumStateMachine::Transition.new(@object, @machine, :shift_down, :first_gear, :idling)
      ],
      [
        EnumStateMachine::Transition.new(@object, @machine, :shift_up, :parked, :idling)
      ],
